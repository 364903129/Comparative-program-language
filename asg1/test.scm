...
;;helper-function for interpret-print
...
(define (interpret-print statement linelist)
        ;; when printables exist, not only keyword "print" 
        ...
        (when (> (length linelist) 1)                 
              (interpret-program (cdr linelist))))        
...
(define (interpret-if statement linelist)
        ...
        (cond ((evaluate-expression relop-expression) 
          (interpret-program (get-label label)))
        ;;continue reading the file
        (else 
          (when (> (length linelist) 1)                 
...
; (define (interpret--input statement linelist)
...
(define (interpret-input statement linelist)
  ...
  (store-input-variables variables linelist)
...
(define (store-input-variables variables linelist)
...
;(printf "EOF: key: ~s, value: ~s\n" 'eof (get-variable 'eof))
  ...
  (when (not (null? (cdr variables)))
    ...
    (store-input-variables remaining-var linelist)
  )
...
(define (interpret-program linelist)
        ...
        (when (= (length line) 2)
              (if (pair? (cadr line))  
                  ...
                  (which-statement (cadr line) linelist) 
                  ;;continue reading     
                  (when (> (length linelist) 1)               
                        (interpret-program (cdr linelist))))) 
